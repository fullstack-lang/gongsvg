...
func (backRepoCircle *BackRepoCircleStruct) CommitPhaseTwoInstance(backRepo *BackRepoStruct, idx uint, circle *models.Circle) (Error error) {
	...
	if circleDB, ok := (*backRepoCircle.Map_CircleDBID_CircleDB)[idx]; ok {
		...
		// This loop encodes the slice of pointers circle.CircleAnimations into the back repo.
		// Each back repo instance at the end of the association encode the ID of the association start
		// into a dedicated field for coding the association. The back repo instance is then saved to the db
		for idx, animateAssocEnd := range circle.CircleAnimations {
			...
			animateAssocEnd_DB.Circle_CircleAnimationsDBID.Int64 = int64(circleDB.ID)
			animateAssocEnd_DB.Circle_CircleAnimationsDBID.Valid = true
			animateAssocEnd_DB.Circle_CircleAnimationsDBID_Index.Int64 = int64(idx)
			animateAssocEnd_DB.Circle_CircleAnimationsDBID_Index.Valid = true
...
func (backRepoCircle *BackRepoCircleStruct) CheckoutPhaseTwoInstance(backRepo *BackRepoStruct, circleDB *CircleDB) (Error error) {
	...
	// This loop redeem circle.CircleAnimations in the stage from the encode in the back repo
	// It parses all AnimateDB in the back repo and if the reverse pointer encoding matches the back repo ID
	// it appends the stage instance
	// 1. reset the slice
	circle.CircleAnimations = circle.CircleAnimations[:0]
	// 2. loop all instances in the type in the association end
	for _, animateDB_AssocEnd := range *backRepo.BackRepoAnimate.Map_AnimateDBID_AnimateDB {
		// 3. Does the ID encoding at the end and the ID at the start matches ?
		if animateDB_AssocEnd.Circle_CircleAnimationsDBID.Int64 == int64(circleDB.ID) {
			// 4. fetch the associated instance in the stage
			animate_AssocEnd := (*backRepo.BackRepoAnimate.Map_AnimateDBID_AnimatePtr)[animateDB_AssocEnd.ID]
			// 5. append it the association slice
			circle.CircleAnimations = append(circle.CircleAnimations, animate_AssocEnd)
		}
	}

	// sort the array according to the order
	sort.Slice(circle.CircleAnimations, func(i, j int) bool {
		animateDB_i_ID := (*backRepo.BackRepoAnimate.Map_AnimatePtr_AnimateDBID)[circle.CircleAnimations[i]]
		animateDB_j_ID := (*backRepo.BackRepoAnimate.Map_AnimatePtr_AnimateDBID)[circle.CircleAnimations[j]]
		...
		return animateDB_i.Circle_CircleAnimationsDBID_Index.Int64 < animateDB_j.Circle_CircleAnimationsDBID_Index.Int64
	})
