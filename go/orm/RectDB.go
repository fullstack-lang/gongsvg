...
func (backRepoRect *BackRepoRectStruct) CommitPhaseTwoInstance(backRepo *BackRepoStruct, idx uint, rect *models.Rect) (Error error) {
	...
	if rectDB, ok := (*backRepoRect.Map_RectDBID_RectDB)[idx]; ok {
		...
		// This loop encodes the slice of pointers rect.RectAnimations into the back repo.
		// Each back repo instance at the end of the association encode the ID of the association start
		// into a dedicated field for coding the association. The back repo instance is then saved to the db
		for idx, animateAssocEnd := range rect.RectAnimations {
			...
			animateAssocEnd_DB.Rect_RectAnimationsDBID.Int64 = int64(rectDB.ID)
			animateAssocEnd_DB.Rect_RectAnimationsDBID.Valid = true
			animateAssocEnd_DB.Rect_RectAnimationsDBID_Index.Int64 = int64(idx)
			animateAssocEnd_DB.Rect_RectAnimationsDBID_Index.Valid = true
...
func (backRepoRect *BackRepoRectStruct) CheckoutPhaseTwoInstance(backRepo *BackRepoStruct, rectDB *RectDB) (Error error) {
	...
	// This loop redeem rect.RectAnimations in the stage from the encode in the back repo
	// It parses all AnimateDB in the back repo and if the reverse pointer encoding matches the back repo ID
	// it appends the stage instance
	// 1. reset the slice
	rect.RectAnimations = rect.RectAnimations[:0]
	// 2. loop all instances in the type in the association end
	for _, animateDB_AssocEnd := range *backRepo.BackRepoAnimate.Map_AnimateDBID_AnimateDB {
		// 3. Does the ID encoding at the end and the ID at the start matches ?
		if animateDB_AssocEnd.Rect_RectAnimationsDBID.Int64 == int64(rectDB.ID) {
			// 4. fetch the associated instance in the stage
			animate_AssocEnd := (*backRepo.BackRepoAnimate.Map_AnimateDBID_AnimatePtr)[animateDB_AssocEnd.ID]
			// 5. append it the association slice
			rect.RectAnimations = append(rect.RectAnimations, animate_AssocEnd)
		}
	}

	// sort the array according to the order
	sort.Slice(rect.RectAnimations, func(i, j int) bool {
		animateDB_i_ID := (*backRepo.BackRepoAnimate.Map_AnimatePtr_AnimateDBID)[rect.RectAnimations[i]]
		animateDB_j_ID := (*backRepo.BackRepoAnimate.Map_AnimatePtr_AnimateDBID)[rect.RectAnimations[j]]
		...
		return animateDB_i.Rect_RectAnimationsDBID_Index.Int64 < animateDB_j.Rect_RectAnimationsDBID_Index.Int64
	})
